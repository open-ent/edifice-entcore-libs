...
public class RepositoryHandler implements Handler<Message<JsonObject>> {
	...
	public void handle(Message<JsonObject> message)
	{
		...
		final String pathPrefix = Server.getPathPrefix(Config.getConf());
		...
		switch (action)
		{
			...
			case "export" :
				...
				if (!Utils.isEmpty(pathPrefix) && exportApps.contains(pathPrefix.substring(1))) {
					...
					lockName = "export_" + exportId + "_" + pathPrefix;
					executor.ensureSingle(lockName, () -> {
			...
			case "import" :
				...
				if (!Utils.isEmpty(appTitle) && importApps.containsKey(appTitle)) {
                    ...
                    lockName = "import_" + importId + "_" + appTitle;
                    executor.ensureSingle(lockName, () -> {
                        ...
                        try {
                            ...
                            final String folderPath = path + File.separator + importApps.getJsonObject(appTitle).getString("folder");
				...
				break;
			case "delete-groups" :
                final JsonArray groups = message.body().getJsonArray("old-groups", new fr.wseduc.webutils.collections.JsonArray());
                lockName = appTitle + "_delete-groups_" + groups.encode().hashCode();
                executor.ensureSingle(lockName, () -> {
                    repositoryEvents.deleteGroups(groups);
                });
                break;
			case "delete-users" :
                final JsonArray users = message.body().getJsonArray("old-users", new fr.wseduc.webutils.collections.JsonArray());
                lockName = appTitle + "_delete-users_" + users.encode().hashCode();
				executor.ensureSingle(lockName, () -> {
					repositoryEvents.deleteUsers(users);
				});
				break;
			case "users-classes-update" :
                final JsonArray updates = message.body().getJsonArray("users-classes-update", new fr.wseduc.webutils.collections.JsonArray());
                lockName = appTitle + "_users-classes-update_" + updates.encode().hashCode();
                executor.ensureSingle(lockName, () -> {
					repositoryEvents.usersClassesUpdated(updates);
				});
				break;
			case "transition" :
                final JsonObject structure = message.body().getJsonObject("structure");
                lockName = appTitle + "_transition_" + structure.encode().hashCode();
				executor.ensureSingle(lockName, () -> {
					repositoryEvents.transition(structure);
				});
				break;
			case "merge-users":
                JsonObject body2 = message.body();
                lockName = appTitle + "_merge-users_" + body2.encode().hashCode();
				executor.ensureSingle(lockName, () -> {
					repositoryEvents.mergeUsers(body2.getString("keepedUserId"), body2.getString("deletedUserId"));
				});
				break;
			case "remove-share-groups":
                final JsonArray oldGroups = message.body().getJsonArray("old-groups", new fr.wseduc.webutils.collections.JsonArray());
                lockName = appTitle + "_remove-share-groups_" + oldGroups.encode().hashCode();
				executor.ensureSingle(lockName, () -> {
					repositoryEvents.removeShareGroups(oldGroups);
				});
				break;
			case "tenants-structures-update":
                final JsonObject body3 = message.body();
                lockName = appTitle + "_tenants-structures-update_" + body3.encode().hashCode();
                executor.ensureSingle(lockName, () -> {
					final JsonArray addedTenantsStructures = body3.getJsonArray("added", new JsonArray());
					final JsonArray deletedTenantsStructures = body3.getJsonArray("deleted", new JsonArray());
			...
			case "timetable-import":
                ...
                lockName = appTitle + "_timetable-import_" + uai;
				executor.ensureSingle(lockName, () -> {
